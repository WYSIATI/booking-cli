 ...
 *
 * This module owns all tool wiring and exports `buildServer` so tests can
 * exercise the full MCP surface in-memory; the stdio process lives in server.ts.
 */
...
export const buildServer = (): McpServer => {
  const server = new McpServer({ name: 'booking-cli', version: VERSION })
